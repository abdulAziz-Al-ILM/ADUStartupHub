...
app.get('/sw.js', (req, res) => {
    ...
    // Majburiy yangilash uchun Service Worker
    res.send(`
...
// ==========================================
// 🔐 WEB API: MUSTAQIL LOGIN (Profil uchun)
// ==========================================
app.post('/api/auth/send-otp', async (req, res) => {
    const { email } = req.body;
    if (!email || (!email.endsWith('@adu.uz') && !email.endsWith('@gmail.com'))) return res.status(400).json({ error: "Faqat @adu.uz qabul qilinadi" });
    const otp = generateOTP();
    
    let user = await prisma.user.findUnique({ where: { email } });
    // TelegramId siz yaratish (schema.prisma da telegramId ga ? qo'yilgan bo'lishi kerak)
...
app.post('/api/auth/verify', async (req, res) => {
    ...
    } else res.status(400).json({ error: "Kod xato!" });
});

// ==========================================
// UMUMIY DIZAYN VA STYLES (Head)
// ==========================================
const headElements = `
    ...
    try {
        ...
        <body class="min-h-screen flex flex-col relative"><div class="header-glow"></div>
            <nav class="w-full card-light fixed top-0 z-50 px-6 py-4 flex justify-between items-center bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
                <div class="flex items-center gap-3"><img src="${LOGO_URL}" class="w-8 h-8"><span class="text-xl font-bold tracking-tight">ADU Hub</span></div>
        ...
        <body class="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-900">
            ...
            <div id="authGateway" class="fixed inset-0 bg-slate-900/80 z-[100] flex flex-col items-center justify-center p-4 backdrop-blur-sm">
                <div class="card-light p-8 rounded-2xl max-w-sm w-full text-center shadow-xl">
                    ...
                    <input id="loginEmail" type="email" placeholder="Pochta manzili" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 mb-4 focus:border-brand text-sm">
                    <input id="loginCode" type="number" placeholder="4 xonali kod (OTP)" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 mb-6 focus:border-brand text-sm hidden">
            ...
            <main class="flex-1 h-full overflow-y-auto p-4 pb-24 md:p-8 z-10">
                ...
                <div id="profile" class="tab-content max-w-5xl mx-auto">
                    ...
                    <div class="card-light p-6 rounded-xl border-t-4 border-brand mb-6 flex justify-between items-center">
                        ...
                        <button onclick="logout()" class="px-4 py-2 bg-red-50 text-red-600 rounded-lg text-sm font-semibold hover:bg-red-100 transition"><i class="fas fa-sign-out-alt"></i> Chiqish</button>
                    </div>
                </div>
                ...
                <div id="problems" class="tab-content max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ${problems.map(pr => `<div class="card-light p-5 rounded-xl"><p class="text-slate-700 dark:text-slate-300 text-sm font-medium">"${pr.description}"</p></div>`).join('')}
                    </div>
                </div>
            </main>
            ...
            <nav class="md:hidden card-light fixed bottom-0 w-full z-50 border-t border-slate-200 dark:border-slate-800 pb-safe">
                <div class="flex justify-around items-center p-2">
                    <button onclick="switchTab('dashboard', this)" class="nav-btn p-2 text-brand active-tab flex flex-col items-center"><i class="fas fa-chart-pie text-lg mb-1"></i><span class="text-[10px]">Tahlil</span></button>
                    <button onclick="switchTab('startups', this)" class="nav-btn p-2 text-slate-400 flex flex-col items-center"><i class="fas fa-rocket text-lg mb-1"></i><span class="text-[10px]">Loyiha</span></button>
                    <button onclick="switchTab('talents', this)" class="nav-btn p-2 text-slate-400 flex flex-col items-center"><i class="fas fa-user-astronaut text-lg mb-1"></i><span class="text-[10px]">Kadr</span></button>
                    <button onclick="switchTab('problems', this)" class="nav-btn p-2 text-slate-400 flex flex-col items-center"><i class="fas fa-fire text-lg mb-1"></i><span class="text-[10px]">Muammo</span></button>
                    <button onclick="switchTab('profile', this)" class="nav-btn p-2 text-slate-400 flex flex-col items-center"><i class="fas fa-user text-lg mb-1"></i><span class="text-[10px]">Profil</span></button>
                </div>
            </nav>

            ${installScript}
            <script>
                // LOGIKA: PROFIL VA KABINET
