...
app.post('/api/admin/request-otp', async (req, res) => {
    const { tgId } = req.body;
    // Xavfsizlik: Barcha belgilar aralashtirilgan 20 xonali kod
    ...
    try {
        if(process.env.TELEGRAM_BOT_TOKEN) {
            ...
            console.log("⚠️ BOT TOKEN YO'Q! Admin kodi:", code); // Bot yo'qligi uchun konsolga chiqaramiz
        }
...
app.post('/api/admin/verify', (req, res) => {
    ...
    if(record && record.code === code && Date.now() < record.expires) {
        delete adminOtpStorage[tgId];
        res.json({ success: true, settings: globalSettings });
    } else {
        res.status(400).json({ error: "Kod xato yoki eskirgan!" });
    }
});

app.post('/api/admin/update', (req, res) => {
    // Haqiqiy loyihada bu yerda token tekshiriladi
    globalSettings = { ...globalSettings, ...req.body };
    res.json({ success: true, settings: globalSettings });
});

// ==========================================
// 🔐 WEB API (Oddiy Foydalanuvchilar)
// ==========================================
app.post('/api/auth/send-otp', async (req, res) => {
    if(!globalSettings.loginAccessEnabled && !globalSettings.emergencyAccess) {
        return res.status(403).json({ error: "Tizimga kirish vaqtincha yopilgan!" });
    }
...
app.post('/api/auth/verify', async (req, res) => {
    ...
    if (email === 'admin@adu.uz' && code === '7777') return res.json({ success: true, email }); // Vaqtinchalik eshik
    if (globalSettings.emergencyAccess && code === '9999') return res.json({ success: true, email }); // Favqulodda eshik
...
app.post('/api/add-item', async (req, res) => {
    ...
    try {
        if (type === 'project') await prisma.project.create({ data: { title: data.title, problemCause: data.cause, goal: data.goal, benefits: data.category, authorId: user.id, groupLink: data.link } }); // benefits orqali sohani vaqtincha saqlaymiz
...
const headElements = `
    ...
    <style>
        ...
        .hover-card { transition: all 0.3s ease; } .hover-card:hover { transform: translateY(-3px); }
        ...
        .border-divider { border-color: #e2e8f0; } .dark .border-divider { border-color: rgba(255,255,255,0.08); }
        @media print { aside, nav, .install-btn, .mobile-nav-item, button { display: none !important; } body { background: white !important; color: black !important; padding: 0; margin: 0; } main { padding: 0 !important; width: 100% !important; } .card-base { box-shadow: none !important; border: 1px solid #cbd5e1 !important; background: white !important;} .text-primary, .text-secondary { color: black !important; } .dark .text-primary, .dark .text-secondary { color: black !important; } }
        ...
        <body class="min-h-screen flex flex-col relative">
            <nav class="w-full card-base fixed top-0 z-50 px-6 py-4 flex justify-between items-center rounded-none border-t-0 border-l-0 border-r-0 border-b border-divider">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <button class="install-btn bg-brand text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-brandhover transition">Ilovani olish</button>
                </div>
            </nav>
            <main class="flex-1 flex flex-col items-center justify-center text-center px-4 pt-32 pb-20 z-10">
                ...
                <div class="flex flex-wrap justify-center gap-6 mb-12">
                    ...
                    <div class="card-base px-10 py-6 rounded-2xl flex flex-col items-center min-w-[160px] border-b-4 border-b-brand"><span class="text-4xl font-black text-brand">${activeProjects}</span><span class="text-[11px] uppercase text-secondary font-bold mt-2 tracking-widest">Startaplar</span></div>
                </div>
...
app.get('/admin', (req, res) => {
    ...
    <body class="h-screen flex items-center justify-center p-4">
        <div id="adminAuth" class="card-base w-full max-w-sm p-8 rounded-2xl text-center">
        ...
        <div id="adminPanel" class="card-base w-full max-w-2xl p-8 rounded-2xl hidden">
        ...
        <script>
            ...
            async function requestAdminCode() {
                ...
                if(codeInput.classList.contains('hidden')) {
                    ...
                    if(res.ok) {
                        ...
                        document.getElementById('adminErr').innerText = "Kod xato yoki muddati tugagan (20 sek)";
                        document.getElementById('adminErr').classList.remove('hidden');
                    }
                }
            }
            function toggleSetting(key) {
                settings[key] = !settings[key]; updateUI();
            }
            function updateUI() {
            ...
            async function saveAdminSettings() {
                ...
                await fetch('/api/admin/update', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(settings) });
                alert("Saqlandi!");
            }
        </script>
    </body></html>
        ...
        <body class="flex h-screen overflow-hidden">
            ...
            <div id="authGateway" class="fixed inset-0 bg-slate-100/90 dark:bg-darkbg/95 z-[100] flex flex-col items-center justify-center p-4 backdrop-blur-sm">
                <div class="card-base p-10 rounded-[2rem] max-w-sm w-full text-center shadow-xl">
                    <div class="w-16 h-16 rounded-2xl bg-brand flex items-center justify-center text-white shadow-lg mx-auto mb-6"><i class="fas fa-rocket text-3xl"></i></div>
                    ...
                    <input id="loginEmail" type="email" placeholder="Pochta manzili (@adu.uz)" class="w-full bg-slate-50 dark:bg-slate-800 border border-divider rounded-xl px-5 py-3.5 mb-4 focus:outline-none focus:border-brand font-medium transition text-primary">
                    <input id="loginCode" type="number" placeholder="4 xonali kod (OTP)" class="w-full bg-slate-50 dark:bg-slate-800 border border-divider rounded-xl px-5 py-3.5 mb-6 focus:outline-none focus:border-brand font-medium hidden transition text-primary">
            ...
            <aside class="w-[280px] card-base h-full flex flex-col hidden md:flex z-20 border-t-0 border-l-0 border-b-0 rounded-none">
                <div class="p-6 flex items-center gap-3 border-b border-divider">
                    <div class="w-8 h-8 rounded-lg bg-brand flex items-center justify-center text-white"><i class="fas fa-rocket text-sm"></i></div>
            ...
            <main class="flex-1 h-full overflow-y-auto p-4 pb-28 md:p-10 z-10">
                ...
                <div id="profile" class="tab-content max-w-5xl mx-auto">
                    ...
                    <div class="card-base p-8 rounded-[2rem] border-t-4 border-t-brand mb-8 flex flex-col md:flex-row md:justify-between items-start md:items-center gap-6">
                        <div class="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <span class="text-[11px] font-bold px-3 py-1 rounded-md bg-emerald-500/10 text-emerald-600 mt-2 inline-block uppercase tracking-widest border border-emerald-500/20">Tasdiqlangan</span>
                            </div>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                            <button class="install-btn px-6 py-3 bg-brand text-white rounded-xl text-sm font-bold hover:bg-brandhover transition w-full md:w-auto"><i class="fas fa-download mr-2"></i> Ilovani O'rnatish</button>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="card-base p-6 rounded-2xl">
                            ...
                            <div id="myProjectsList" class="space-y-3">
                                <p class="text-xs text-secondary italic">Sizda hali startap yo'q.</p>
                            </div>
                        </div>
                        <div class="card-base p-6 rounded-2xl">
                            ...
                            <div id="myResumesList" class="space-y-3">
                                <p class="text-xs text-secondary italic">Sizda hali rezyume yo'q.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="dashboard" class="tab-content active max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                        <div class="card-base p-6 rounded-2xl border-t-4 border-t-brand"><p class="text-secondary text-[11px] font-bold uppercase tracking-widest mb-2">Umumiy A'zolar</p><h3 class="text-4xl font-black text-primary">${totalUsers}</h3></div>
                        <div class="card-base p-6 rounded-2xl border-t-4 border-t-amber-500"><p class="text-secondary text-[11px] font-bold uppercase tracking-widest mb-2">Jami Startaplar</p><h3 class="text-4xl font-black text-primary">${activeProjects.length}</h3></div>
                        <div class="card-base p-6 rounded-2xl border-t-4 border-t-purple-500"><p class="text-secondary text-[11px] font-bold uppercase tracking-widest mb-2">Kadrlar (Rezyume)</p><h3 class="text-4xl font-black text-primary">${resumes.length}</h3></div>
                        <div class="card-base p-6 rounded-2xl border-t-4 border-t-rose-500"><p class="text-secondary text-[11px] font-bold uppercase tracking-widest mb-2">Kashfiyotlar</p><h3 class="text-4xl font-black text-primary">${problems.length}</h3></div>
                    </div>
                </div>
                ...
                <div id="talents" class="tab-content max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ${resumes.map(r => {
                            let parts = r.skills.split(':');
                            let cat = parts[0]; let desc = parts.slice(1).join(':') || r.skills;
                            return `
                            <div class="card-base p-6 rounded-2xl border-l-4 border-l-purple-500 hover-card">
                                <div class="flex justify-between items-center mb-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400"><i class="fas fa-user-circle text-sm"></i></div>
                                        <span class="font-bold text-primary text-sm">Talaba #${r.id} (Yashirin)</span>
                                    </div>
                                    <span class="text-[10px] uppercase tracking-widest text-purple-600 font-bold px-2 py-1 rounded bg-purple-500/10">${safeHTML(cat)}</span>
                                </div>
                                <p class="text-sm text-secondary leading-relaxed">${safeHTML(desc)}</p>
                                <button onclick="alert('Tez orada Resume to\\'liq ko\\'rinishi mumkin bo\\'ladi')" class="text-xs font-bold text-brand mt-4 underline">Batafsil ko'rish</button>
                            </div>
                        `}).join('')}
                    </div>
                </div>
            ...
            <nav class="md:hidden card-base fixed bottom-0 left-0 w-full z-50 rounded-none border-t border-b-0 border-l-0 border-r-0 pb-safe">
            ...
            <div id="detailModal" class="fixed inset-0 bg-slate-900/60 z-[60] hidden items-center justify-center p-4 backdrop-blur-sm">
                <div class="card-base w-full max-w-2xl rounded-2xl p-8 md:p-10 relative shadow-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <a id="modalActionBtn" href="#" target="_blank" class="bg-brand hover:bg-brandhover text-white w-full flex items-center justify-center gap-3 py-4 text-sm font-bold rounded-xl transition shadow-sm">
                        <i class="fab fa-telegram-plane"></i> Jamoaga qo'shilish so'rovi (Yashirin Profil)
                    </a>
                </div>
            </div>
            ...
            <script>
                ...
                // Shaxsiy profilni to'ldirish
                function renderMyItems(email) {
                    const myProjHtml = myProjectsRaw.filter(p => p.author && p.author.email === email).map(p => 
                        \`<div class="flex justify-between items-center p-3 border border-divider rounded-lg">
                            <span class="text-sm font-bold text-primary">\${p.title}</span>
                            <button onclick="deleteItem('project', \${p.id})" class="text-xs text-rose-500 font-bold px-2 py-1 bg-rose-50 dark:bg-rose-900/20 rounded">O'chirish</button>
                        </div>\`
                    ).join('');
                    document.getElementById('myProjectsList').innerHTML = myProjHtml || "<p class='text-xs text-secondary italic'>Startap yo'q.</p>";
                    ...
                    const myResHtml = myResumesRaw.filter(r => r.author && r.author.email === email).map(r => 
                        \`<div class="flex justify-between items-center p-3 border border-divider rounded-lg">
                            ...
                            <button onclick="deleteItem('resume', \${r.id})" class="text-xs text-rose-500 font-bold px-2 py-1 bg-rose-50 dark:bg-rose-900/20 rounded">O'chirish</button>
                        </div>\`
                    ).join('');
                    document.getElementById('myResumesList').innerHTML = myResHtml || "<p class='text-xs text-secondary italic'>Rezyume yo'q.</p>";
                }
                ...
                async function checkLogin() {
                    ...
                    if(!waitingForOTP) {
                        ...
                        else { err.innerText = "Kod xato yoki Favqulodda ruxsat yopiq!"; err.classList.remove('hidden'); btn.innerText = "Tasdiqlash"; }
                    }
                }
                ...
                function openWebForm(type) {
                    ...
                    if(type === 'project') {
                        ...
                        content.innerHTML = \`
                            ...
                            <details class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl cursor-pointer border border-brand/10">
                                <summary class="font-bold text-brand text-sm">❗ Yopiq guruh linki (Muhim)</summary>
                                <p class="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">Sizga kelgan arizalarni qabul qilish uchun Telegramda yopiq guruh (Private Group) oching va bu yerga uning Invite Link'ini qo'ying.</p>
                            </details>
                            <input id="fLink" placeholder="https://t.me/+AbCdEfG" class="\${inputClass} !mb-0">\`;
                    } else if(type === 'resume') {
                        document.getElementById('formTitle').innerText = "Rezyume Qo'shish";
                        content.innerHTML = \`
                            <select id="fCategory" class="\${inputClass}">\${categoriesSelect}</select>
                            <textarea id="fSkills" placeholder="Qobiliyatlaringiz va o'zingiz haqingizda" class="\${inputClass} h-32 resize-none !mb-0"></textarea>\`;
                    } else {
