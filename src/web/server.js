...
// ==========================================
// 🎨 EDITORIAL NATURE DIZAYN (Premium)
// ==========================================
const headElements = `
    <link rel="manifest" href="/app.webmanifest?v=nature_premium">
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <script>
        tailwind.config = { darkMode: 'class', theme: { extend: { colors: { brand: '#10B981', brandhover: '#059669', darkbase: '#021812' } } } }
    ...
    <style>
        ...
        body { font-family: 'Inter', sans-serif; transition: background-color 0.5s ease, color 0.5s ease; -webkit-tap-highlight-color: transparent; }
        .title-font { font-family: 'Playfair Display', serif; letter-spacing: -0.02em; }
        
        /* ------------------------------------- */
        /* ☀️ LIGHT MODE (Nafis Oq-Yashil) */
        /* ------------------------------------- */
        body:not(.dark) { background-color: #F0FDF4; color: #064E3B; } 
        body:not(.dark) .grid-bg { background-image: radial-gradient(rgba(16, 185, 129, 0.1) 1px, transparent 1px); }
        body:not(.dark) .ambient-glow { background: radial-gradient(circle at 50% 0%, rgba(52, 211, 153, 0.2) 0%, transparent 60%); filter: blur(60px); }
        body:not(.dark) .card-base { background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(24px); border: 1px solid rgba(255, 255, 255, 0.9); box-shadow: 0 10px 40px -10px rgba(16, 185, 129, 0.1); }
        body:not(.dark) .thematic-row { background: rgba(255, 255, 255, 0.9); border: 1px solid rgba(16, 185, 129, 0.1); }
        body:not(.dark) .thematic-img-mask { background: linear-gradient(to right, rgba(255,255,255,1) 30%, rgba(255,255,255,0.2) 70%, transparent 100%); }

        /* ------------------------------------- */
        /* 🌙 DARK MODE (Chuqur O'rmon) */
        /* ------------------------------------- */
        .dark body { background-color: #02120D; color: #ECFDF5; } 
        .dark .grid-bg { background-image: radial-gradient(rgba(16, 185, 129, 0.05) 1px, transparent 1px); }
        .dark .ambient-glow { background: radial-gradient(circle at 50% 0%, rgba(16, 185, 129, 0.15) 0%, transparent 60%); filter: blur(60px); }
        .dark .card-base { background: rgba(6, 30, 22, 0.7); backdrop-filter: blur(24px); border: 1px solid rgba(16, 185, 129, 0.1); box-shadow: 0 20px 40px -10px rgba(0, 0, 0, 0.5); }
        .dark .thematic-row { background: rgba(4, 20, 15, 0.9); border: 1px solid rgba(16, 185, 129, 0.15); }
        .dark .thematic-img-mask { background: linear-gradient(to right, rgba(4,20,15,1) 40%, rgba(4,20,15,0.4) 70%, transparent 100%); }
        
        /* ------------------------------------- */
        /* 🖼 THEMATIC LIST STYLE (Ro'yxat ichidagi muhit) */
        /* ------------------------------------- */
        ...
        .thematic-row:hover { transform: translateY(-4px); box-shadow: 0 20px 25px -5px rgba(16, 185, 129, 0.2); }
        ...
        .thematic-row:hover .thematic-img { transform: scale(1.05); }
        
        .thematic-img-mask {
            position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1; pointer-events: none;
        }
        .thematic-content { position: relative; z-index: 2; width: 75%; }
...
// 🟢 DINAMIK RASM TANLOVCHI (Kategoriya bo'yicha)
function getCategoryData(text) {
    let lower = text.toLowerCase();
    if(lower.includes("dizayn") || lower.includes("figma") || lower.includes("ui")) return { cat: "Dizayn & San'at", color: "purple", img: "https://images.unsplash.com/photo-1561070791266-9654fca64f9c?auto=format&fit=crop&w=600&q=80" };
    if(lower.includes("sotuv") || lower.includes("menejer") || lower.includes("biznes")) return { cat: "Biznes & Sotuv", color: "amber", img: "https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=600&q=80" };
    if(lower.includes("smm") || lower.includes("marketing") || lower.includes("target")) return { cat: "Marketing", color: "rose", img: "https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?auto=format&fit=crop&w=600&q=80" };
    return { cat: "Muhandislik & IT", color: "blue", img: "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=600&q=80" };
}

// 🍃 STARTAPLAR UCHUN TABIAT/TEXNOLOGIYA RASMLARI
const startupImgs = [
    "https://images.unsplash.com/photo-1501854140801-50d01698950b?auto=format&fit=crop&w=600&q=80", // Nature/Forest
    "https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=600&q=80", // Earth/Space
    "https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=600&q=80", // Tech Board
    "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=600&q=80"  // Cyber Security
];

const ROCKET_ICON = `<div class="w-8 h-8 rounded-xl bg-gradient-to-tr from-brand to-emerald-400 flex items-center justify-center text-white shadow-lg"><i class="fas fa-leaf text-sm"></i></div>`;
const ROCKET_ICON_LARGE = `<div class="w-16 h-16 rounded-2xl bg-gradient-to-tr from-brand to-emerald-400 flex items-center justify-center text-white shadow-xl shadow-brand/40 mx-auto mb-5"><i class="fas fa-leaf text-3xl"></i></div>`;
...
app.get('/', async (req, res) => {
    try {
        ...
        res.send(`
        <!DOCTYPE html><html lang="uz"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>ADU Hub | Ekotizim</title>${headElements}</head>
        <body class="min-h-screen flex flex-col relative">
            ...
            <nav class="w-full card-base fixed top-0 z-50 px-6 py-4 flex justify-between items-center border-b border-brand/10">
                <div class="flex items-center gap-3">${ROCKET_ICON}<span class="text-xl title-font font-bold tracking-tight">ADU Ekotizimi</span></div>
                <div class="flex gap-4">
                    <button onclick="toggleTheme()" class="text-emerald-600 dark:text-emerald-400 hover:text-brand transition"><i class="fas fa-moon dark:hidden"></i><i class="fas fa-sun hidden dark:block"></i></button>
            ...
            <main class="flex-1 flex flex-col items-center justify-center text-center px-4 pt-32 pb-20 z-10">
                <div class="mb-6 inline-block px-5 py-1.5 rounded-full bg-brand/10 text-brand font-bold text-[10px] uppercase tracking-widest border border-brand/20 shadow-sm">Yopiq Ekotizim</div>
                <h1 class="text-5xl md:text-7xl font-black mb-6 leading-tight max-w-4xl mx-auto title-font text-slate-900 dark:text-white">Tabiiy Iqtidorlardan <br/><span class="text-transparent bg-clip-text bg-gradient-to-r from-brand to-emerald-400 italic">Global Startaplarga.</span></h1>
                <p class="text-lg md:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mb-12 leading-relaxed">Universitetning yagona aqlli platformasi. O'z g'oyangizni ekin kabi ekib, katta loyihaga aylantiring.</p>
                <div class="flex flex-wrap justify-center gap-6 mb-12">
                    <div class="card-base px-10 py-6 rounded-3xl flex flex-col items-center min-w-[160px] border-t-4 border-slate-300 dark:border-slate-700"><span class="text-4xl font-black title-font text-slate-900 dark:text-white">${totalUsers}</span><span class="text-[10px] uppercase text-slate-500 font-bold mt-2 tracking-widest">A'zolar</span></div>
                    <div class="card-base px-10 py-6 rounded-3xl flex flex-col items-center min-w-[160px] border-t-4 border-brand"><span class="text-4xl font-black title-font text-brand">${activeProjects}</span><span class="text-[10px] uppercase text-slate-500 font-bold mt-2 tracking-widest">Startaplar</span></div>
                </div>
                <a href="/app" class="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-10 py-5 rounded-2xl font-bold text-lg transition-all shadow-2xl hover:scale-105 flex items-center gap-3">Platformaga kirish <i class="fas fa-arrow-right"></i></a>
            </main>
            ${installScript}
        </body></html>
        `);
...
// ==========================================
// 2-XONA: ILOVA VA KABINET (Thematic Lists)
// ==========================================
app.get('/app', async (req, res) => {
    try {
        ...
        <body class="flex h-screen overflow-hidden relative">
            ...
            <div id="authGateway" class="fixed inset-0 bg-white/40 dark:bg-darkbase/80 z-[100] flex flex-col items-center justify-center p-4 backdrop-blur-2xl">
                <div class="card-base p-10 rounded-[2.5rem] max-w-sm w-full text-center shadow-2xl">
                    ${ROCKET_ICON_LARGE}
                    <h2 class="text-3xl title-font font-bold mb-2 text-slate-900 dark:text-white">Tizimga kirish</h2>
            ...
            <aside class="w-[280px] card-base m-4 rounded-[2rem] h-[calc(100vh-32px)] flex flex-col hidden md:flex z-20 shadow-2xl overflow-hidden">
                <div class="p-6 flex items-center gap-3 border-b border-brand/10">
                    ${ROCKET_ICON}
                    <h1 class="text-xl title-font font-bold text-slate-900 dark:text-white">ADU Ekotizimi</h1>
                </div>
                <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                    ...
                    <button onclick="switchTab('talents', this)" class="nav-btn w-full flex items-center gap-4 px-5 py-3.5 text-slate-600 dark:text-slate-400 hover:bg-brand/10 hover:text-brand rounded-2xl font-semibold transition-all"><i class="fas fa-dna w-5 text-center"></i> Kadrlar</button>
                ...
                <div class="p-6 border-t border-brand/10 flex flex-col gap-3">
                    ...
                    <button onclick="toggleTheme()" class="w-full flex items-center justify-center gap-3 px-4 py-3.5 text-emerald-600 dark:text-emerald-400 hover:bg-brand/10 rounded-2xl font-semibold transition"><i class="fas fa-moon dark:hidden"></i><i class="fas fa-sun hidden dark:block"></i> Mavzu</button>
                </div>
            </aside>
            ...
            <main class="flex-1 h-full overflow-y-auto p-4 pb-28 md:p-8 z-10">
                <div class="md:hidden flex justify-between items-center mb-8 card-base p-4 rounded-2xl shadow-sm">
                    <div class="flex items-center gap-3">${ROCKET_ICON}<span class="title-font font-bold text-lg text-slate-900 dark:text-white">ADU Ekotizimi</span></div>
                    <div class="flex gap-3">
                        <button onclick="toggleTheme()" class="text-emerald-600 dark:text-emerald-400 w-10 h-10 flex items-center justify-center rounded-xl bg-brand/5"><i class="fas fa-moon dark:hidden"></i><i class="fas fa-sun hidden dark:block"></i></button>
                ...
                <div id="profile" class="tab-content max-w-5xl mx-auto">
                    <h2 class="text-4xl title-font font-bold mb-8 text-slate-900 dark:text-white">Shaxsiy Kabinet</h2>
                    <div class="card-base p-8 rounded-[2rem] border-t-4 border-brand mb-8 flex flex-col md:flex-row md:justify-between items-start md:items-center gap-6">
                        <div class="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <span class="text-[10px] font-bold px-3 py-1 rounded-md bg-brand text-white mt-2 inline-block uppercase tracking-widest">Tasdiqlangan</span>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div onclick="openWebForm('project')" class="card-base border-2 border-dashed border-brand/30 hover:border-brand cursor-pointer p-8 rounded-[2rem] flex flex-col items-center justify-center text-center transition group"><i class="fas fa-seedling text-3xl text-brand/50 group-hover:text-brand mb-3 transition"></i><p class="font-bold text-slate-700 dark:text-slate-300 group-hover:text-brand">Startap ekish</p></div>
                        ...
                        <div onclick="openWebForm('problem')" class="card-base border-2 border-dashed border-amber-500/30 hover:border-amber-500 cursor-pointer p-8 rounded-[2rem] flex flex-col items-center justify-center text-center transition group"><i class="fas fa-bolt text-3xl text-amber-500/50 group-hover:text-amber-500 mb-3 transition"></i><p class="font-bold text-slate-700 dark:text-slate-300 group-hover:text-amber-500">Muammo yozish</p></div>
                    </div>
                </div>

                <div id="dashboard" class="tab-content active max-w-6xl mx-auto">
                    <h2 class="text-4xl title-font font-bold mb-8 text-slate-900 dark:text-white">Ekotizim Tahlili</h2>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
                        <div class="card-base p-8 rounded-[2rem]"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2">A'zolar</p><h3 class="text-4xl title-font font-black text-slate-900 dark:text-white">${totalUsers}</h3></div>
                        <div class="card-base p-8 rounded-[2rem]"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2">G'oya bosqichi</p><h3 class="text-4xl title-font font-black text-amber-500">${teamBuilding}</h3></div>
                        <div class="card-base p-8 rounded-[2rem]"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2">MVP bosqichi</p><h3 class="text-4xl title-font font-black text-purple-500">${mvpStage}</h3></div>
                        <div class="card-base p-8 rounded-[2rem] border-b-4 border-brand"><p class="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2">Bozorda</p><h3 class="text-4xl title-font font-black text-brand">${launched}</h3></div>
                    </div>
                </div>

                <div id="startups" class="tab-content max-w-6xl mx-auto">
                    <h2 class="text-4xl title-font font-bold mb-8 text-slate-900 dark:text-white">Loyiha O'rmoni</h2>
                    <div class="flex flex-col gap-6">
                        ${activeProjects.length > 0 ? activeProjects.map((p, index) => {
                            ...
                            <div class="thematic-row group cursor-pointer" onclick="openModal('${safeHTML(p.title)}', '${safeHTML(p.problemCause)}', '${safeHTML(p.goal)}', '${safeHTML(p.benefits)}', '${p.id}')">
                                <img src="${imgUrl}" class="thematic-img">
                                <div class="thematic-img-mask"></div>
                                <div class="thematic-content p-6 md:p-8 flex flex-col justify-center h-full w-full md:w-3/4">
                                    <span class="text-[10px] uppercase tracking-widest text-brand font-bold mb-3 inline-block px-3 py-1 rounded-md bg-brand/10 border border-brand/20 w-max">Startap</span>
                                    <h3 class="title-font text-3xl font-bold mb-3 text-slate-900 dark:text-white leading-tight">${safeHTML(p.title)}</h3>
                                    <p class="text-sm md:text-base text-slate-700 dark:text-slate-300 line-clamp-2 md:line-clamp-3 leading-relaxed max-w-2xl">${safeHTML(p.goal)}</p>
                                </div>
                            </div>
                ...
                <div id="talents" class="tab-content max-w-6xl mx-auto">
                    <h2 class="text-4xl title-font font-bold mb-8 text-slate-900 dark:text-white">Noyob Iqtidorlar</h2>
                    <div class="flex flex-col gap-6">
                        ${resumes.map(r => {
                            const catData = getCategoryData(r.skills);
                            return `
                            <div class="thematic-row group">
                                <img src="${catData.img}" class="thematic-img">
                                <div class="thematic-img-mask"></div>
                                <div class="thematic-content p-6 md:p-8 flex flex-col justify-center h-full w-full md:w-3/4">
                                    <div class="flex items-center gap-3 mb-3">
                                        <i class="fas fa-user-astronaut text-${catData.color}-500 text-xl"></i>
                                        <span class="font-bold text-slate-900 dark:text-white text-lg">@${r.author.username || 'yashirin_kadr'}</span>
                                    </div>
                                    <span class="text-[10px] uppercase tracking-widest text-${catData.color}-600 dark:text-${catData.color}-400 font-bold mb-3 inline-block px-3 py-1 rounded-md bg-${catData.color}-500/10 border border-${catData.color}-500/20 w-max">${catData.cat}</span>
                                    <p class="text-sm md:text-base text-slate-700 dark:text-slate-300 leading-relaxed max-w-2xl">${safeHTML(r.skills)}</p>
                                </div>
                            </div>
                        `}).join('')}
                    </div>
                ...
                <div id="problems" class="tab-content max-w-6xl mx-auto">
                    <h2 class="text-4xl title-font font-bold mb-8 text-slate-900 dark:text-white">Kashfiyotlar (Muammolar)</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ${problems.map(pr => `<div class="card-base p-8 rounded-[2rem] relative border-t-4 border-amber-500"><i class="fas fa-bolt text-4xl text-amber-500/20 absolute top-6 right-6"></i><p class="text-slate-800 dark:text-slate-200 text-lg font-medium leading-relaxed relative z-10 pt-2 pr-8">"${safeHTML(pr.description)}"</p><div class="mt-6 pt-4 border-t border-brand/10"><span class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Anonim Kuzatuvchi</span></div></div>`).join('')}
                    </div>
                </div>
            </main>
            ...
            <div id="webFormModal" class="fixed inset-0 bg-slate-900/60 z-[70] hidden items-center justify-center p-4 backdrop-blur-md">
                <div class="card-base w-full max-w-lg rounded-[2.5rem] p-8 relative shadow-2xl">
                    ...
                    <h2 id="formTitle" class="text-2xl title-font font-bold mb-6 text-slate-900 dark:text-white">Ma'lumot qo'shish</h2>
            ...
            <div id="detailModal" class="fixed inset-0 bg-slate-900/60 z-[60] hidden items-center justify-center p-4 backdrop-blur-lg">
                <div class="card-base w-full max-w-2xl rounded-[3rem] p-8 md:p-12 relative shadow-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <h2 id="modalTitle" class="text-3xl md:text-4xl title-font font-black mb-8 pr-12 leading-tight text-slate-900 dark:text-white">Sarlavha</h2>
            ...
            <script>
                ...
                function openWebForm(type) {
                    ...
                    if(type === 'project') {
                        ...
                        content.innerHTML = '<textarea id="fSkills" placeholder="O\\'zingiz haqingizda va qobiliyatlaringiz (Sotuv, Dasturlash, Dizayn...)" class="w-full bg-slate-50/50 dark:bg-slate-800/50 border border-brand/20 p-4 rounded-2xl text-sm h-32 focus:outline-none focus:border-brand font-medium resize-none text-slate-900 dark:text-white"></textarea>';
                    } else {
