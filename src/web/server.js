...
// ==========================================
let globalSettings = {
    loginAccessEnabled: true, // Tizimga ulana olish huquqi
    aiDailyLimit: 10,         // AI so'rovlari limiti
    emergencyAccess: false    // Favqulodda eshik
};
let adminOtpStorage = {};     // Admin parollari saqlanadigan xotira

// ==========================================
// 🚀 RAKETA LOGOTIPI (Mukammal o'lchamli PWA SVG)
// ==========================================
app.get('/icon.svg', (req, res) => {
    res.setHeader('Content-Type', 'image/svg+xml');
    // scale(0.8) yordamida rasm markazga qotirildi, telefon qirqib tashlamaydi
...
app.post('/api/admin/request-otp', async (req, res) => {
    ...
    // Kod faqatgina 20 soniya yashaydi
    adminOtpStorage[tgId] = { code, expires: Date.now() + 20000 };
    
    try {
        if(process.env.TELEGRAM_BOT_TOKEN) {
            await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`, {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ chat_id: tgId, text: `🔒 *ADMIN KOD:*\n\n\`${code}\`\n\n_Ushbu kod faqat 20 soniya amal qiladi!_`, parse_mode: 'Markdown' })
            });
        }
        res.json({ success: true });
    } catch(e) { res.status(500).json({ error: "Telegram botga ulana olmadi" }); }
});

app.post('/api/admin/verify', (req, res) => {
    const { tgId, code } = req.body;
    const record = adminOtpStorage[tgId];
    if(record && record.code === code && Date.now() < record.expires) {
        delete adminOtpStorage[tgId]; // ishlatilgan kod o'chiriladi
...
// ==========================================
// 🔐 WEB API (Foydalanuvchilar UCHUN)
// ==========================================
app.post('/api/auth/send-otp', async (req, res) => {
    const { email } = req.body;
    if(email === 'admin@adu.uz') return res.json({ success: true }); // Admin eshigi sirkulyarni aylanib o'tadi
    
    if(!globalSettings.loginAccessEnabled && !globalSettings.emergencyAccess) {
        return res.status(403).json({ error: "Tizimga yangi a'zolar kirishi vaqtincha yopilgan!" });
    }
...
app.post('/api/auth/verify', async (req, res) => {
    ...
    // 1. Yashirin Admin Eshigi
    if (email === 'admin@adu.uz' && code === '7777') {
        let adminUser = await prisma.user.findUnique({ where: { email: 'admin@adu.uz' } });
        if(!adminUser) await prisma.user.create({ data: { email: 'admin@adu.uz', isVerified: true, telegramId: BigInt(123456789) } });
        return res.json({ success: true, email }); 
    }
    
    // 2. Favqulodda Eshik (Faqat admindan ruxsat bo'lsa ishlaydi)
    if (globalSettings.emergencyAccess && code === '9999') {
        let user = await prisma.user.findUnique({ where: { email } });
        if (!user) user = await prisma.user.create({ data: { email, isVerified: true, telegramId: BigInt(Date.now()) } });
        else await prisma.user.update({ where: { email }, data: { isVerified: true, otpCode: null } });
        return res.json({ success: true, email });
    }

    // 3. Oddiy Foydalanuvchi Tekshiruvi
...
app.post('/api/add-item', async (req, res) => {
    ...
    const user = await prisma.user.findUnique({ where: { email } });
    if (!user) return res.status(401).json({ error: "Avtorizatsiya xatosi, sahifani yangilab qaytadan kiring." });
    ...
    try {
        ...
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: "Server ma'lumotni saqlay olmadi" }); }
});
...
app.post('/api/delete-item', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true });
    } catch(e) { res.status(500).json({ error: "O'chirishda xatolik yuz berdi" }); }
});

// ==========================================
// 🎨 UMUMIY CSS VA DIZAYN (Clean SaaS)
// ==========================================
const headElements = `
    ...
    <style>
        ...
        .dark .card-custom { background: #151e32; border: 1px solid #334155; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.2); }
        .dark .text-primary { color: #e2e8f0; } .dark .text-secondary { color: #94a3b8; }
        ...
        .border-divider { border-color: #e2e8f0; } .dark .border-divider { border-color: #334155; }
...
// 🧠 Kadrlar va Startaplar uchun 8 ta aniq Kategoriya
function getCategory(text) {
    let lower = text.toLowerCase();
    if (lower.includes("dizayn")) return { cat: "Dizayn va Media", color: "purple" };
    if (lower.includes("sotuv")) return { cat: "Sotuv va Biznes", color: "amber" };
    if (lower.includes("marketing")) return { cat: "Marketing va PR", color: "rose" };
    if (lower.includes("ekologiya")) return { cat: "Ekologiya va Agro", color: "emerald" };
    if (lower.includes("tibbiyot")) return { cat: "Tibbiyot", color: "red" };
    if (lower.includes("ta'lim")) return { cat: "Ta'lim", color: "indigo" };
    if (lower.includes("it va dasturlash")) return { cat: "Dasturlash", color: "brand" };
        ...
        <body class="min-h-screen flex flex-col relative bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
            ...
            <nav class="w-full card-custom fixed top-0 z-50 px-6 py-4 flex justify-between items-center rounded-none border-t-0 border-l-0 border-r-0">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <button class="install-btn install-btn-nav bg-brand text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-brandhover transition">Ilovani olish</button>
                </div>
            </nav>
...
app.get('/admin', (req, res) => {
    res.send(`
    <!DOCTYPE html><html lang="uz"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Admin Panel</title>${headElements}</head>
    <body class="h-screen flex items-center justify-center p-4">
        <div id="adminAuth" class="card-custom w-full max-w-sm p-8 rounded-2xl text-center">
            ...
            <p class="text-sm text-secondary mb-6">Telegram ID ni kiriting</p>
            <input id="tgId" type="number" placeholder="Sizning Telegram ID" class="w-full bg-slate-50 dark:bg-slate-800 border border-divider rounded-xl px-4 py-3 mb-4 text-primary focus:outline-none focus:border-brand">
            <input id="adminCode" type="text" placeholder="20 xonali kod" class="w-full bg-slate-50 dark:bg-slate-800 border border-divider rounded-xl px-4 py-3 mb-6 text-primary focus:outline-none focus:border-brand hidden">
            <button id="adminBtn" onclick="requestAdminCode()" class="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-3 rounded-xl font-bold">Kod jo'natish</button>
            <p id="adminErr" class="text-rose-500 text-sm mt-3 font-bold hidden"></p>
        </div>

        <div id="adminPanel" class="card-custom w-full max-w-2xl p-8 rounded-2xl hidden">
            ...
            <div class="flex items-center justify-between mb-6">
                <div><h4 class="font-bold text-primary">Favqulodda eshik</h4><p class="text-xs text-secondary">Parolsiz (9999) kirish huquqi</p></div>
            ...
            <div class="flex items-center justify-between mb-6 border-t border-divider pt-6">
                <div><h4 class="font-bold text-primary">Sun'iy Intellekt Limiti</h4><p class="text-xs text-secondary">Kunlik har bir foydalanuvchiga</p></div>
                <input id="limitVal" type="number" class="w-20 bg-slate-50 dark:bg-slate-800 border border-divider rounded-lg px-3 py-2 text-primary text-center">
            </div>
            
            <button onclick="saveAdminSettings()" class="w-full bg-emerald-500 text-white py-3 rounded-xl font-bold mt-4">Saqlash</button>
        </div>
        <script>
            let currentTgId = ""; let settings = {};
            async function requestAdminCode() {
                const tgId = document.getElementById('tgId').value;
                const codeInput = document.getElementById('adminCode');
                const btn = document.getElementById('adminBtn');
                if(!tgId) return; currentTgId = tgId;
                
                if(codeInput.classList.contains('hidden')) {
                    btn.innerText = "Kuting...";
                    await fetch('/api/admin/request-otp', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({tgId}) });
                    codeInput.classList.remove('hidden'); btn.innerText = "Tasdiqlash";
                } else {
                    const code = codeInput.value;
                    const res = await fetch('/api/admin/verify', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({tgId, code}) });
                    if(res.ok) {
                        const data = await res.json(); settings = data.settings;
                        document.getElementById('adminAuth').classList.add('hidden');
                        document.getElementById('adminPanel').classList.remove('hidden');
                        updateUI();
                    } else { document.getElementById('adminErr').innerText = "Kod xato yoki muddati tugagan (20 sek)"; document.getElementById('adminErr').classList.remove('hidden'); }
                }
            }
    ...
    try {
        ...
        <body class="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
            ...
            <main class="flex-1 h-full overflow-y-auto p-4 pb-28 md:p-10 z-10">
                ...
                <div id="profile" class="tab-content max-w-5xl mx-auto">
                    ...
                    <div class="card-custom p-8 rounded-[2rem] border-t-4 border-t-brand mb-8 flex flex-col md:flex-row md:justify-between items-start md:items-center gap-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                            ...
                            <button onclick="logout()" class="px-6 py-3 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-xl text-sm font-bold hover:bg-rose-100 dark:hover:bg-rose-900/50 transition w-full md:w-auto"><i class="fas fa-sign-out-alt mr-2"></i> Chiqish</button>
                        </div>
                    </div>
                    
                    <h3 class="text-sm font-bold text-slate-500 uppercase tracking-widest mb-5 mt-10 pl-2">Ma'lumot Qo'shish</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        <div onclick="openWebForm('project')" class="card-custom border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-brand cursor-pointer p-6 rounded-2xl flex flex-col items-center justify-center text-center transition hover-card"><i class="fas fa-rocket text-2xl text-slate-400 mb-2 transition"></i><p class="font-bold text-slate-600 dark:text-slate-300">Startap qo'shish</p></div>
                        <div onclick="openWebForm('resume')" class="card-custom border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-purple-500 cursor-pointer p-6 rounded-2xl flex flex-col items-center justify-center text-center transition hover-card"><i class="fas fa-dna text-2xl text-slate-400 mb-2 transition"></i><p class="font-bold text-slate-600 dark:text-slate-300">Rezyume (Kadr)</p></div>
                        <div onclick="openWebForm('problem')" class="card-custom border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-amber-500 cursor-pointer p-6 rounded-2xl flex flex-col items-center justify-center text-center transition hover-card"><i class="fas fa-fire text-2xl text-slate-400 mb-2 transition"></i><p class="font-bold text-slate-600 dark:text-slate-300">Muammo yozish</p></div>
                    </div>

                    <h3 class="text-sm font-bold text-slate-500 uppercase tracking-widest mb-5 pl-2 border-t border-slate-200 dark:border-slate-800 pt-8">Mening Ma'lumotlarimni Boshqarish</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="card-custom p-6 rounded-2xl">
                            <h4 class="font-bold text-slate-900 dark:text-slate-100 mb-4 border-b border-slate-200 dark:border-slate-700 pb-2"><i class="fas fa-rocket text-brand mr-2"></i> Startaplarim</h4>
                            <div id="myProjectsList" class="space-y-3"><p class="text-xs text-slate-500 italic">Sizda hali startap yo'q.</p></div>
                        </div>
                        <div class="card-custom p-6 rounded-2xl">
                            <h4 class="font-bold text-slate-900 dark:text-slate-100 mb-4 border-b border-slate-200 dark:border-slate-700 pb-2"><i class="fas fa-dna text-purple-500 mr-2"></i> Rezyumelarim</h4>
                            <div id="myResumesList" class="space-y-3"><p class="text-xs text-slate-500 italic">Sizda hali rezyume yo'q.</p></div>
                        </div>
                    </div>
                </div>
                ...
                <div id="dashboard" class="tab-content active max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                        <div class="card-custom p-6 rounded-2xl border-t-4 border-t-brand"><p class="text-slate-500 text-[11px] font-bold uppercase tracking-widest mb-2">Umumiy A'zolar</p><h3 class="text-4xl font-black text-slate-900 dark:text-slate-100">${totalUsers}</h3></div>
                        <div class="card-custom p-6 rounded-2xl border-t-4 border-t-amber-500"><p class="text-slate-500 text-[11px] font-bold uppercase tracking-widest mb-2">Jami Startaplar</p><h3 class="text-4xl font-black text-slate-900 dark:text-slate-100">${activeProjects.length}</h3></div>
                        <div class="card-custom p-6 rounded-2xl border-t-4 border-t-purple-500"><p class="text-slate-500 text-[11px] font-bold uppercase tracking-widest mb-2">Kadrlar (Rezyume)</p><h3 class="text-4xl font-black text-slate-900 dark:text-slate-100">${resumes.length}</h3></div>
                        <div class="card-custom p-6 rounded-2xl border-t-4 border-t-rose-500"><p class="text-slate-500 text-[11px] font-bold uppercase tracking-widest mb-2">Kashfiyotlar</p><h3 class="text-4xl font-black text-slate-900 dark:text-slate-100">${problems.length}</h3></div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="card-custom p-6 md:p-8 rounded-2xl">
                            <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-6 border-b border-slate-200 dark:border-slate-700 pb-4 flex items-center"><i class="fas fa-chart-pie text-purple-500 mr-3 text-xl"></i> Sohalar Bo'yicha Iqtidorlar</h3>
                            <div class="mb-5"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Dasturlash (IT)</span><span class="font-bold text-slate-900 dark:text-slate-100">${devCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-brand h-3 rounded-full" style="width: ${(devCount/totalRes*100)||0}%"></div></div></div>
                            <div class="mb-5"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Dizayn & Media</span><span class="font-bold text-slate-900 dark:text-slate-100">${designCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-purple-500 h-3 rounded-full" style="width: ${(designCount/totalRes*100)||0}%"></div></div></div>
                            <div class="mb-5"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Biznes & Sotuv</span><span class="font-bold text-slate-900 dark:text-slate-100">${bizCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-amber-500 h-3 rounded-full" style="width: ${(bizCount/totalRes*100)||0}%"></div></div></div>
                            <div class="mb-2"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Marketing & PR</span><span class="font-bold text-slate-900 dark:text-slate-100">${markCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-rose-500 h-3 rounded-full" style="width: ${(markCount/totalRes*100)||0}%"></div></div></div>
                        </div>

                        <div class="card-custom p-6 md:p-8 rounded-2xl">
                            <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-6 border-b border-slate-200 dark:border-slate-700 pb-4 flex items-center"><i class="fas fa-layer-group text-rose-500 mr-3 text-xl"></i> Startap sohalari</h3>
                            <div class="mb-5"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Dasturlash (IT)</span><span class="font-bold text-slate-900 dark:text-slate-100">${sDevCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-brand h-3 rounded-full" style="width: ${(sDevCount/totalProj*100)||0}%"></div></div></div>
                            <div class="mb-5"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Dizayn & Media</span><span class="font-bold text-slate-900 dark:text-slate-100">${sDesignCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-purple-500 h-3 rounded-full" style="width: ${(sDesignCount/totalProj*100)||0}%"></div></div></div>
                            <div class="mb-5"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Tijorat & Biznes</span><span class="font-bold text-slate-900 dark:text-slate-100">${sBizCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-amber-500 h-3 rounded-full" style="width: ${(sBizCount/totalProj*100)||0}%"></div></div></div>
                            <div class="mb-2"><div class="flex justify-between text-sm mb-2"><span class="font-semibold text-slate-600 dark:text-slate-400">Marketing & Boshqa</span><span class="font-bold text-slate-900 dark:text-slate-100">${sMarkCount} ta</span></div><div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 progress-bar-bg"><div class="bg-rose-500 h-3 rounded-full" style="width: ${(sMarkCount/totalProj*100)||0}%"></div></div></div>
                        </div>
                    </div>
                </div>
                ...
                <div id="startups" class="tab-content max-w-6xl mx-auto">
                    ...
                    <div class="flex flex-col gap-4">
                        ${activeProjects.length > 0 ? activeProjects.map((p) => `
                            <div class="hover-card card-custom p-6 rounded-2xl cursor-pointer group flex flex-col md:flex-row md:items-center justify-between gap-4" onclick="openModal('${safeHTML(p.title)}', '${safeHTML(p.problemCause)}', '${safeHTML(p.goal)}', '${safeHTML(p.benefits)}', '${p.id}', true)">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <span class="text-[10px] uppercase font-bold px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded ml-2 hidden md:block">${safeHTML(p.benefits)}</span>
                                    </div>
                ...
                <div id="talents" class="tab-content max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ${resumes.map(r => {
                            ...
                            return `
                            <div class="card-custom p-5 rounded-2xl border-l-4 border-l-${catData.color}-500 hover-card cursor-pointer" onclick="openModal('Arizachi profili', 'Men haqimda va qobiliyatlarim', '${safeHTML(descText)}', '${safeHTML(catText)}', '${r.id}', false)">
                                <div class="flex justify-between items-center mb-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-bold text-slate-900 dark:text-slate-100 text-sm">Arizachi profili</span>
                                    </div>
                                ...
                                <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-2">${safeHTML(descText)}</p>
                            </div>
                        `}).join('')}
                    </div>
                </div>
                ...
                <div class="card-custom w-full max-w-2xl rounded-2xl p-8 md:p-10 relative shadow-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="space-y-5 mb-10">
                        <div class="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-200 dark:border-slate-700">
                            <h4 id="lbl1" class="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Muammo Sababi</h4>
                            <p id="modalCause" class="text-slate-800 dark:text-slate-200 text-sm leading-relaxed">...</p>
                        </div>
                        <div class="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-200 dark:border-slate-700">
                            <h4 id="lbl2" class="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Asosiy Maqsad</h4>
                            <p id="modalGoal" class="text-slate-800 dark:text-slate-200 text-sm leading-relaxed">...</p>
                        </div>
                        <div id="benBox" class="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-200 dark:border-slate-700 hidden">
                            <h4 id="lbl3" class="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Soha / Kategoriya</h4>
            ...
            <script>
                ...
                if(authEmail) { document.getElementById('authGateway').style.display = 'none'; document.getElementById('userEmailDisplay').innerText = authEmail; renderMyItems(authEmail); }

                function renderMyItems(email) {
                    const myProjHtml = myProjectsRaw.filter(p => p.author && p.author.email === email).map(p => 
                        \`<div class="flex justify-between items-center p-3 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                            <span class="text-sm font-bold text-slate-900 dark:text-slate-100 truncate w-3/4">\${p.title}</span>
                            <button onclick="deleteItem('project', \${p.id})" class="text-[10px] text-rose-500 font-bold px-2 py-1 bg-rose-50 dark:bg-rose-900/30 rounded hover:bg-rose-100 uppercase tracking-widest">O'chirish</button>
                        </div>\`
                    ).join('');
                    document.getElementById('myProjectsList').innerHTML = myProjHtml || "<p class='text-xs text-slate-500 italic'>Sizda startap yo'q.</p>";

                    const myResHtml = myResumesRaw.filter(r => r.author && r.author.email === email).map(r => 
                        \`<div class="flex justify-between items-center p-3 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                            <span class="text-sm font-bold text-slate-900 dark:text-slate-100 truncate w-3/4">\${r.skills.split(':')[0]}</span>
                            <button onclick="deleteItem('resume', \${r.id})" class="text-[10px] text-rose-500 font-bold px-2 py-1 bg-rose-50 dark:bg-rose-900/30 rounded hover:bg-rose-100 uppercase tracking-widest">O'chirish</button>
                        </div>\`
                    ).join('');
                    document.getElementById('myResumesList').innerHTML = myResHtml || "<p class='text-xs text-slate-500 italic'>Sizda rezyume yo'q.</p>";
                }
                ...
                async function checkLogin() {
                    ...
                    if(email === 'admin@adu.uz') {
                        ...
                        // Backend tekshiruvi orqali o'tishi uchun else ga uzatamiz
                    }
                ...
                function openWebForm(type) {
                    ...
                    if(type === 'project') {
                        ...
                        content.innerHTML = \`
                            ...
                            <input id="fCause" placeholder="Muammo sababi (Kim qiynalyapti?)" class="\${inputClass}">
                            <textarea id="fGoal" placeholder="Sizning yechimingiz va maqsadingiz" class="\${inputClass} h-20 resize-none"></textarea>
                            ...
                            <details class="mb-4 p-4 bg-brand/5 rounded-xl cursor-pointer border border-brand/20">
                                ...
                                <p class="text-xs text-slate-600 dark:text-slate-400 mt-3 leading-relaxed">Sizga telegramdan "Jamoaga qo'shilish" so'rovlari kelishi uchun:<br>1. Telegramda yangi guruh oching.<br>2. Guruh turini "Private" (Yopiq) qiling.<br>3. Guruhning Invite Link'ini (Taklif havolasini) nusxalang va pastga joylang.</p>
                            </details>
                            <input id="fLink" placeholder="https://t.me/+A1B2C3D4" class="\${inputClass} !mb-0">\`;
                    } else if(type === 'resume') {
                        document.getElementById('formTitle').innerText = "Rezyume Qo'shish";
                        content.innerHTML = \`
                            <select id="fCategory" class="\${inputClass}">\${categoriesSelect}</select>
                            <textarea id="fSkills" placeholder="O'zingiz haqingizda va qobiliyatlaringiz (Misol: Backend yoza olaman, Figma bilaman...)" class="\${inputClass} h-32 resize-none !mb-0"></textarea>\`;
                    } else {
                        document.getElementById('formTitle').innerText = "Muammo Yozish";
                        content.innerHTML = \`<textarea id="fDesc" placeholder="Sizni nima qiynayapti? Ekotizim bunga yechim topsin (Anonim qoladi)" class="\${inputClass} h-32 resize-none !mb-0"></textarea>\`;
                    }
                ...
                function openModal(title, cause, goal, benefits, id, isProject) {
                    ...
                    if(isProject) {
                        ...
                        // Kadrlar (Resume) uchun oyna
                        document.getElementById('lbl1').innerText = "Mutaxassislik sohasi";
                        document.getElementById('lbl2').innerText = "Haqida / Qobiliyatlari";
