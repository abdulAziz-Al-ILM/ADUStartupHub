...
bot.on('text', async (ctx) => {
    ...
    try {
        ...
        if (state && state.step === 'AWAITING_EMAIL') {
            // Faqat @adu.uz uchun qat'iy tekshiruv
            if (!text.toLowerCase().endsWith('@adu.uz')) {
            ...
            // Agar tasdiqlangan bo'lsa, qolgan menyu logikalari shu yerda davom etadi...
            if (!user.isVerified) return ctx.reply("Iltimos, avval pochtangizni tasdiqlang. (/start)");
            // Qolgan bot ishlari (loyiha qo'shish, rezyume va hokazo bu yerda bo'ladi)
            ctx.reply("Tez orada ushbu bo'limlar ham modulli arxitekturaga ulanadi!");
        }
